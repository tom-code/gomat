...
func flow() {
	...
	tlv3.writeOctetString(4, sha1.New().Sum(public_key)) // subject-key-id
	tlv3.writeOctetString(5, sha1.New().Sum(public_key)) // authority-key-id
	tlv3.writeAnonStructEnd()
	...
	//log.Println(hex.EncodeToString(tt))
	log.Println(hex.EncodeToString(tlv3.data.Bytes()))

	t := tlvdec.Decode(tlv3.data.Bytes())
	t.Dump(0)
	var tlv4 TLVBuffer
	tlv4.writeOctetString(0, tlv3.data.Bytes())
	//tlv4.writeOctetString(0, tt)
